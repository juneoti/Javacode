...
public class MemberMain02 {
	...
	private static void selectContact() {
		...
		for (int i = 0; i < ((MemberDAOImple) dao).getCount(); i++) {
	...
	private static void selectIndexContact() {
		...
		*/
		if (index >= 0 && index < ((MemberDAOImple) dao).getCount()) {
	...
	private static void updateContact() {
		...
		*/
		MemberVO vo = dao.select(index);
		vo = new MemberVO("", pw, name, email, email, null);
	...
	private static void deleteContact() {
		...
		if(((MemberDAOImple) dao).getCount() == 0) {
		...
		int index = sc.nextInt();
		MemberVO vo = dao.select(index);
			if(index < ((MemberDAOImple) dao).getCount()) {
				System.out.println(vo);
				...
				if(yesNo.equals("yes")) {
					...
					for (int i = index; i < ((MemberDAOImple) dao).getCount(); i++) {
						dao.delete(index, vo);
					}
					delete = false;
				}
