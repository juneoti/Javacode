...
public class MemberMain {
	...
	private static MemberVO[] memberList = new MemberVO[100];
	...
	private static void insertContact() {
		...
		if(count >= memberList.length) {
			System.out.println("더 이상 회원을 저장할 수 없습니다.");
			return;
		}
	}
	...
	private static void detailContact() {
		...
		// 등록된 연락처가 있을경우 검색하고 싶은 연락처의 인덱스를 입력하면 그 연락처를 출력
			...
			if(index < count) {
				System.out.println(memberList[index].toString());
			}
	...
	private static void editContact() {
		...
		// 등록된 연락처가 있을경우 수정하고 싶은 연락처의 인덱스를 입력하면 그 연락처를 출력 및 수정
			...
			int index = sc.nextInt();
			for(int i = index; i < memberList.length;) {
			if(index >= 0 && index < count) {
				...
				memberList[index].setPw(pw);
				memberList[index].setName(name);
				memberList[index].setEmail(email);
				memberList[index].setInterest(interest);
				}
			// 입력한 인덱스값이 등록된 연락처의 인덱스 값을 넘을 경우 출력
			else {
				System.out.println("마지막 인덱스 숫자보다 작은 숫자를 입력해주세요.");
	...
	private static void deleteContact() {
		...
		int index = sc.nextInt();
			if(index < count) {
				System.out.println(memberList[index].toString());
				System.out.println("해당 회원 정보의 삭제를 원한다면 yes를, 취소하려면 no를 입력하세요.");
	...
	public static void main(String[] args) {
		// 시작 (프로그램의 이름)
		System.out.println("회원관리 프로그램 version 0.1");
		boolean run = true;
