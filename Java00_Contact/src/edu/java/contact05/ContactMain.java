...
import edu.java.contact04.ContactDAO;
import edu.java.contact04.ContactDAOImple;
...
public class ContactMain {
   ...
   private void initialize() {
	  ...
	  // JLabel 타이틀 설정
      JLabel lblTitle = new JLabel("연락처 프로그램 Version 0.5");
   ...
   private void insertContact() {
		...
		if(result == 1) {
			txtAreaLog.setText("등록된 연락처 개수 : " + ((ContactDAOImple) dao).getSize() + "\n" + "연락처 등록 완료");
			selectAllContactTable();
		}
		}
   }
   ...
   private void selectContact() {
	   ...
	   if (index >= 0 && index < ((ContactDAOImple) dao).getSize()) {
		   ...
		   txtAreaLog.setText(index + "번 인덱스 연락처 정보");
	   }else {
